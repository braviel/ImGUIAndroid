...
ImGuiIO::ImGuiIO()
{
    ...
    IniSavingRate = 5.0f;
    IniFilename = "/sdcard/imgui/imgui.txt";
    LogFilename = "/sdcard/imgui/imgui_log.txt";
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar) && !(flags & ImGuiWindowFlags_NoCollapse))
        {
			...
			if (g.HoveredWindow == window && IsMouseHoveringRect(title_bar_rect.Min, title_bar_rect.Max) && g.IO.MouseON[0])
            {
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (hovered)
    {
		/*LOGI("hover %d", hovered);
		LOGI("bb %.2f, %.2f  %.2f, %.2f", bb.Min.x, bb.Min.y, bb.Max.x, bb.Max.y);
		LOGI("cursor %.2f %.2f", g.IO.MousePos.x, g.IO.MousePos.y);*/
		//LOGI("hovered %d", hovered);
        SetHoveredID(id);
    ...
    if (g.ActiveId == id)
    {
		//LOGI("g.IO.MouseDown[0] %d", g.IO.MouseDown[0]);
		//LOGI("g.IO.preHovered %d", g.IO.preHovered);
        if (g.IO.MouseDown[0])
        {
            held = true;
        }
